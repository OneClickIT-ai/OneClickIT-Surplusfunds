...
async function getCounties(params: DirectoryParams) {
...
export default async function DirectoryPage({ searchParams }: PageProps) {
  ...
  const { counties, total, page, totalPages } = await getCounties(params);
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link
                href={`/directory?${new URLSearchParams({ ...params, page: String(page - 1) })}`}
